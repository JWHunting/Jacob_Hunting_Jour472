...
```{r message=FALSE}
md_city_income <- read_csv("city_md_income.csv") %>% 
  as.data.frame()
```
...
```{r}
highest_pct <-md_city_income %>% 
  mutate(absolute_change = median_inc_2020 - median_inc_2010,
         percentage_change = abs(((median_inc_2020 - median_inc_2010)/(median_inc_2010))*100))%>% 
  arrange(desc(percentage_change))
```

Beaver Creek has the highest absolute change from 2010 to 2020 with an absolute change of 118292. Shaft CDP had the highest percentage change from 2010 to 2020 with a percentage change of 399.87%

Produce a table with the top 25 places ranked by the highest percentage change.

```{r}
 pct_25 <- md_city_income %>% 
...
```{r}
summary(md_city_income$median_inc_2010)
